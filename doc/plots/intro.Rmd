...
As a help for plotting basic plots like boxplots of glycan data
**glycanr** package can be used.

To install it we first need to install **devtools** package.
...
Now we can load **devtools** package.
...
Devtools package has a function to help us install packages from GitHub.
Using this function we will install **glycanr** package.
...
First we need to create a *data.frame* to simulate our glycan data.

```{r}
n <- 200
X <- data.frame(ID=1:n, GP1=runif(n), GP2=rexp(n, 0.3), GP3=rgamma(n, 2), cc=factor(sample(1:2, n, replace=TRUE)))
```

Now we have data.frame *X* where *GP* represents glycans, *ID* represents e.g. sample IDs
and *cc* represents Case/Control status.
